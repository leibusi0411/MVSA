...
#split='train-standard'  读取的文件: places365_train_standard.txt
#split='val'   读取的文件: places365_val.txt
#没有公开测试集的标签 torchvision 的 Places365 类没有提供加载测试集的选项
...
class Places365(VisionDataset):
    ...
    # 加载类别元数据   读取 categories_places365.txt
    ...
    # 加载文件列表元数据   读取 places365_train_standard.txt 
    def load_file_list(self, download: bool = True) -> Tuple[List[Tuple[str, int]], List[int]]:
        def process(line: str, sep="/") -> Tuple[str, int]:
         ...
         # 根据 self.split (如 'train-standard') 获取对应的文件名和MD5
        file, md5 = self._FILE_LIST_META[self.split]
        file = path.join(self.root, file)
        if not self._check_integrity(file, md5, download):
            self.download_devkit()

        # 读取文件列表，并处理每一行，生成 (完整图像路径, 整数标签) 的元组列表
        with open(file) as fh:
            images = [process(line) for line in fh]

        _, targets = zip(*images)
        return images, list(targets) #它返回 self.imgs（一个 (路径, 标签) 元组的列表）和 self.targets（一个纯标签的列表）
