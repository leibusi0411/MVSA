...
def get_two_stage_config(config: dict) -> dict:
    ...
    # 阶段二 teacher 更新模式：ema 或 periodic
    target_update_mode = str(two_stage.get('target_update_mode', 'ema'))

    # 周期刷新间隔（仅 periodic 模式使用，单位：epoch）
    target_update_interval_epochs = max(
        1,
        int(two_stage.get('target_update_interval_epochs', 1))
    )

    # EMA 动量（仅 ema 模式使用，per-batch 更新）
    ema_momentum = float(two_stage.get('ema_momentum', 0.999))

    # Centering 动量（teacher logits 中心向量的 EMA 动量）
    centering_momentum = float(two_stage.get('centering_momentum', 0.9))

    # 调试参数：gt_mode 控制标签注入方式
    #   teacher: 用真实标签替代 teacher 目标
    #   local:   对 local_views 直接做 cross-entropy 监督
    # gt_interval 控制替换间隔
    #   0: 仅在 warmup+3 时替换一次
    #   1-10: 每隔 N 个 epoch 替换一次
    #   >10: 仅在 warmup+3 时替换一次
    gt_mode = str(two_stage.get('gt_mode', 'teacher'))
    gt_interval = int(two_stage.get('gt_interval', 0))
    ...
    return {
        ...
        'centering_momentum': centering_momentum,
...
def build_unsupervised_model_name(dataset_name: str, config: dict) -> str:
    ...
    update_mode = two_stage_cfg['target_update_mode']
    if update_mode == 'ema':
        mode_suffix = f"mema{two_stage_cfg['ema_momentum']:.4f}"
    else:
        mode_suffix = f"mperiodic_u{two_stage_cfg['target_update_interval_epochs']}"
    loss_suffix = (
...
def init_ema_teacher(student_model_without_ddp, device: torch.device):
    """
    初始化 EMA teacher 模型。
    架构与 student 相同，CLIP 参数共享（冻结），只维护 STN 可训练参数的 EMA。
    """
    from copy import deepcopy
    from stn.multi_view_stn import MultiViewSTNModel

    # 创建 teacher：共享冻结的 CLIP 主干，仅复制 STN 可训练组件
    teacher = MultiViewSTNModel(
        clip_model=student_model_without_ddp.clip_model,
        config=deepcopy(student_model_without_ddp.config),
        num_views=student_model_without_ddp.num_views,
    ).to(device)
    teacher = teacher.float()

    # 只复制 STN 参数（localization_network + fusion_module），CLIP 参数共享引用
    student_state = student_model_without_ddp.state_dict()
    teacher_state = {k: v for k, v in student_state.items()
                     if k.startswith('localization_network.') or k.startswith('fusion_module.')}
    ...
    # 预缓存可训练参数的 teacher/student 指针对，避免每 batch 遍历全部参数
    teacher._ema_param_pairs = [
        (t_p, s_p) for t_p, s_p in zip(teacher.parameters(), student_model_without_ddp.parameters())
        if s_p.requires_grad
    ]
    return teacher


def update_ema_teacher(teacher, ema_momentum: float):
    """
    每个 batch 用 EMA 更新 teacher 的可训练参数。
    使用预缓存的 _ema_param_pairs 避免遍历全部参数。
    """
    pairs = getattr(teacher, '_ema_param_pairs', None)
    if pairs is None:
        return
    with torch.no_grad():
        for t_p, s_p in pairs:
            t_p.data.mul_(ema_momentum).add_(s_p.data, alpha=1.0 - ema_momentum)
...
def update_centering_buffer(c: torch.Tensor, teacher_logits: torch.Tensor,
                            momentum: float) -> torch.Tensor:
    """
    EMA 更新中心向量: c = momentum * c + (1-momentum) * mean(logits)
    teacher_logits: [B, num_classes]，对 batch 维度求平均
    DDP 场景下对 batch_mean 做 all-reduce，保证各 rank 中心向量一致。
    """
    with torch.no_grad():
...
def compute_teacher_target_probs(teacher_model,
                                 images_448: torch.Tensor,
                                 text_features: torch.Tensor,
                                 target_temp: float,
                                 center_c: torch.Tensor | None = None) -> tuple:
    """
    使用 teacher 模型计算目标分布。支持 Centering：
    teacher_logits → 减去中心向量 c → 温度锐化 → softmax
    返回 (target_probs, teacher_logits)，teacher_logits 用于更新 centering buffer。
    """
    with torch.no_grad():
        ...
        if center_c is not None:
            centered_logits = teacher_logits - center_c.unsqueeze(0)
        else:
            centered_logits = teacher_logits
        target_probs = F.softmax(centered_logits / max(target_temp, 1e-6), dim=-1)
    return target_probs, teacher_logits
...
def compute_two_stage_unsupervised_loss(criterion,
    ...
    - 阶段二（teacher-guided）：teacher 分布监督 Local，最小化 KL(teacher || local_views)
      teacher 更新方式由 two_stage.target_update_mode 控制：ema（每 batch EMA）或 periodic（按 epoch 刷新）
    """
    ...
    if epoch < two_stage_cfg['warmup_epochs']:
        ...
        loss_details['phase'] = 'dec_teacher'
    ...
    if getattr(criterion, 'decorrelation_loss', None) is not None and getattr(criterion, 'decorrelation_weight', 0.0) > 0:
        dec = criterion.decorrelation_loss(view_features, normalized=True)
    ...
    if getattr(criterion, 'fairness_loss', None) is not None and getattr(criterion, 'fairness_weight', 0.0) > 0:
        fair = criterion.fairness_loss(view_features, text_features, normalized=True)
...
def unsupervised_train_one_epoch(model: DDP,
    ...
    """
    无监督训练一个epoch。
    当 teacher_model 不为 None 时每 batch 执行 EMA 更新，
    当 center_c 不为 None 时对 teacher logits 做 centering 并更新中心向量。
    """
    model.train()
    total_loss_sum = 0.0
    total_correct = 0
    ...
    for batch_idx, (images, labels) in enumerate(iterable):
        ...
        # 硬编码：改2 — CE(fused, labels)，从 warmup+3 开始一直替换
        gt_offset = 3
        gt_interval = 1
        gt_mode = 'local'
        use_gt = False
        if epoch >= two_stage_cfg['warmup_epochs'] + gt_offset:
            stage2_epoch = epoch - two_stage_cfg['warmup_epochs']
            if gt_interval <= 0:
                # 只替换一次：warmup + gt_offset
                use_gt = (epoch == two_stage_cfg['warmup_epochs'] + gt_offset)
            elif gt_interval <= 10:
                # 每隔 N 个 epoch 替换（从 warmup + gt_offset 开始）
                use_gt = ((stage2_epoch - gt_offset) % gt_interval == 0)
            else:
                # >10: 只替换一次
                use_gt = (epoch == two_stage_cfg['warmup_epochs'] + gt_offset)
        ...
        if use_gt:
            if gt_mode == 'teacher':
                ...
                teacher_target_probs = F.one_hot(labels, num_classes).float()
                if is_main_process() and batch_idx == 0:
                    print(f"\n🔬 [DEBUG-teacher] Epoch {epoch+1}: 用真实标签替代 teacher\n")
            # gt_mode=='local' 不在此处处理，loss 计算时直接加 CE
        if not use_gt or gt_mode != 'teacher':
            if (
                epoch >= two_stage_cfg['warmup_epochs']
                and teacher_model is not None
            ):
                teacher_target_probs, teacher_logits = compute_teacher_target_probs(
                ...
                if center_c is not None:
                    update_centering_buffer(center_c, teacher_logits, centering_momentum)
        ...
        loss, loss_details, fused_logits_raw = compute_two_stage_unsupervised_loss(
            ...
            config=config,
            periodic_target_probs=teacher_target_probs,
        )

        # 仅用于监控准确率
        logits_for_acc = fused_logits_raw / temperature

        # DEBUG local 模式：loss 改为 CE(fused, labels)，同时保留 KL 值用于报告
        if use_gt and gt_mode == 'local':
            kl_loss = loss.detach().item()  # 保存原始 KL loss 值
            loss = F.cross_entropy(fused_logits_raw.float(), labels)
            if is_main_process() and batch_idx == 0:
                print(f"\n🔬 [DEBUG-local] Epoch {epoch+1}: CE={loss.item():.4f} (原 KL={kl_loss:.4f})\n")

        loss.backward()

        # 梯度裁剪（使用预缓存的参数列表）
        if trainable_params:
            torch.nn.utils.clip_grad_norm_(trainable_params, max_norm=max_grad_norm, norm_type=grad_norm_type)

        optimizer.step()

        # EMA 更新 teacher（阶段二每 batch 执行）
        if teacher_model is not None and ema_momentum < 1.0:
            update_ema_teacher(teacher_model, ema_momentum)
        ...
        if is_main_process() and hasattr(iterable, 'set_postfix'):
            ...
            elif loss_details.get('phase') == 'dec_teacher':
            ...
            if center_c is not None:
                progress_dict['cNorm'] = f'{center_c.norm().item():.2f}'
    ...
    with torch.no_grad():
        ...
        for batch_idx, (images, labels) in enumerate(iterable):
            ...
            if (
                ...
                teacher_target_probs, _ = compute_teacher_target_probs(
                    teacher_model=teacher_model,
                    images_448=images,
                    text_features=text_features,
                    target_temp=two_stage_cfg['dec_target_temp'],
            ...
            # DEBUG: CE 模式下验证损失用 CE，同时报告 KL 值
            if epoch >= two_stage_cfg['warmup_epochs'] + 3:
                kl_loss_val = loss.detach().item()
                loss = F.cross_entropy(fused_logits_raw.float(), labels)
                loss_details['kl_original'] = kl_loss_val
...
def main():
    ...
    # 读取配置（支持绝对路径、相对路径、UN-STN-Config目录）
    if args.config:
    ...
    # 设置无监督训练
    with suppress_stdout_if_not_main():
        criterion, optimizer, scheduler, trainable_params = setup_unsupervised_training(stn_model, config, device)
    ...
    if is_main_process():
        ...
        if update_mode == 'ema':
            print(f"    * 阶段二(EMA teacher): 温度(T_target={two_stage_cfg['dec_target_temp']}, T_student={two_stage_cfg['dec_student_temp']}), KL权重={stage2_kl_weight}, EMA动量={two_stage_cfg['ema_momentum']:.4f}")
        else:
            print(f"    * 阶段二(周期目标): 温度(T_target={two_stage_cfg['dec_target_temp']}, T_student={two_stage_cfg['dec_student_temp']}), KL权重={stage2_kl_weight}, 刷新间隔={two_stage_cfg['target_update_interval_epochs']} epochs")
        print(f"    * Centering: 动量={centering_momentum:.2f}, 中心向量维度={num_classes}")
        print(f"  - 额外正则:")
    ...
    teacher_model = None
    ema_momentum = two_stage_cfg['ema_momentum']
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        # 阶段二 teacher 管理
        if epoch >= two_stage_cfg['warmup_epochs']:
            if two_stage_cfg['target_update_mode'] == 'ema':
                # EMA 模式：首次进入阶段二时初始化 teacher，后续每 batch 自动更新
                if teacher_model is None:
                    teacher_model = init_ema_teacher(model_without_ddp, device)
                    if is_main_process():
                        print(f"🔧 EMA teacher 已初始化: epoch={epoch+1}, momentum={ema_momentum:.4f}")
            else:
                # 周期性刷新模式（兼容旧配置）
                ...
                if should_refresh:
                    # 复用 init_ema_teacher 做首次/周期全量复制
                    teacher_model = init_ema_teacher(model_without_ddp, device)
                    if is_main_process():
                        print(
        ...
        # EMA 模式下每 batch 自动更新，周期性模式下 ema_momentum=1.0 则跳过更新
        effective_ema_momentum = ema_momentum if two_stage_cfg['target_update_mode'] == 'ema' else 1.0
        ...
        if is_main_process():
            ...
            if val_acc > best_val_acc:
                ...
                print(f"  🎯 验证准确率改善 -{improvement:.6f}! 新最佳Acc: {best_val_acc:.3f}, Loss: {best_acc_loss:.6f} ({best_acc_epoch}轮)")
                print(f"  💾 已保存最佳模型: {ckpt_best_loss}")
            else:
                patience_counter += 1
                print(f"  ⏳ 验证准确率未改善 {patience_counter}/{patience} 轮")

            if patience_counter >= patience:
                print("🛑 早停触发，结束训练")

        # 同步早停决定（所有 rank 必须统一执行，不可放在 if is_main_process 内）
        should_stop = False
        if is_dist_avail_and_initialized():
        ...
        # 等待 rank 0 完成可能的模型保存，避免下次 collective 操作超时
        if is_dist_avail_and_initialized():
            dist.barrier()
    ...
    if is_main_process():
        print("\n🎉 无监督分布式训练完成")
        print(f"🏆 最佳模型: Epoch {best_acc_epoch}, Val Acc={best_val_acc:.3f}, Val Loss={best_acc_loss:.6f}")
