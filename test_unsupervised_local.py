"""
无监督多视角STN模型测试脚本（含局部视角加权评估）

新增功能：基于 WCA (Weighted Cross Alignment) 方法，使用 STN 局部视角特征
与原始全局特征的相似度对局部 logits 进行加权聚合，评估 STN 定位学习的有效性。

WCA 温度参数 wca_temperature 从配置文件 stn_config.wca_temperature 读取，默认 0.07。

使用示例：
  python test_unsupervised_local.py --dataset_name oxford_pets
  python test_unsupervised_local.py --dataset_name oxford_pets --ckpt_path checkpoints/unsupervised/oxford_pets/xxx_best.pth
  python test_unsupervised_local.py --dataset_name oxford_pets --list
"""
...
def compute_wca_weighted_logits(view_features, original_features, text_features, temperature=0.07):
    """
    基于 WCA 方法计算局部视角加权 logits。

    步骤：
    1. 计算每个局部视角与原始全局特征的余弦相似度
    2. softmax + 温度缩放得到权重
    3. 用权重加权聚合各视角 logits

    Args:
        view_features: [B, N, D] STN局部视角特征（L2归一化）
        original_features: [B, D] 原始全局特征（L2归一化）
        text_features: [D, C] 文本特征（L2归一化）
        temperature: 相似度温度参数

    Returns:
        weighted_logits: [B, C] 加权聚合的局部 logits
    """
    # 计算每个局部视角与原始图像的相似度 [B, N]
    similarities = (view_features * original_features.unsqueeze(1)).sum(dim=-1)

    # softmax + 温度缩放得到权重 [B, N]
    weights = F.softmax(similarities / temperature, dim=-1)

    # 先加权聚合特征 [B, N, D] -> [B, D]，再计算 logits（等价但更省计算）
    weighted_features = (weights.unsqueeze(-1) * view_features).sum(dim=1)
    weighted_logits = weighted_features @ text_features  # [B, D] @ [D, C] -> [B, C]
...
def visualize_preprocessed_vs_stn(preprocessed_images, transformed_images, dataset_name,
                                   config=None,
                                   save_dir="visualizations/test_unsupervised_local_transforms",
...
def stn_precise_testing(stn_model, dataloader, device, dataset_name,
    ...
    with torch.no_grad():
        for batch_idx, (images_448, labels) in enumerate(tqdm(dataloader, desc="Testing")):
            ...
            # 提取局部视角特征用于 WCA 加权
            view_features = vis_data.get('view_features')
            original_features = vis_data.get('original_features')

            if view_features is not None and original_features is not None:
                all_view_features.append(view_features)
                all_original_features.append(original_features)

            if batch_idx < visual_batches:
                try:
                    save_base = "visualizations/test_unsupervised_local_transforms"
                    if model_tag:
            ...
            all_fused_features.append(batch_features)
            all_targets.append(labels)

    all_fused_features = torch.cat(all_fused_features, dim=0)
    all_targets = torch.cat(all_targets, dim=0)
    total_samples = all_fused_features.size(0)

    # 全局融合 logits（原有）
    fused_logits = all_fused_features @ precomputed_text_features
    fused_acc = accuracy(fused_logits, all_targets, total_samples, dataset_name)

    # 局部视角加权 logits（新增 WCA 风格）
    local_acc = None
    if all_view_features:
        all_view_features = torch.cat(all_view_features, dim=0)
        all_original_features = torch.cat(all_original_features, dim=0)
        weighted_logits = compute_wca_weighted_logits(
            all_view_features, all_original_features,
            precomputed_text_features, temperature=wca_temperature
        )
        local_acc = accuracy(weighted_logits, all_targets, total_samples, dataset_name)

    print(f"  Tested {total_samples} samples")
    print(f"  Fused (global) Top-1 Acc:  {fused_acc:.2f}%")
    if local_acc is not None:
        print(f"  Local (WCA-weighted) Acc:  {local_acc:.2f}%")
    return fused_acc, local_acc
...
def extract_ckpt_info(ckpt_path, config):
    ...
    import re
    info = {}
    m = re.search(r'twostage_w(\d+)_', ckpt_path)
    if m:
        info['warmup_epochs'] = int(m.group(1))
    m = re.search(r'_kl([\d.]+)_', ckpt_path)
    if m:
        info['kl_weight'] = float(m.group(1))
    m = re.search(r'_td([\d.]+)_', ckpt_path)
    if m:
...
def main():
    parser = argparse.ArgumentParser(description='无监督多视角STN模型测试（含局部加权评估）')
    ...
    try:
        ...
        # WCA 温度从配置文件读取
        wca_temperature = float(stn_config.get('wca_temperature', 0.07))
        ...
        print(f"WCA 温度:     {wca_temperature}")
        ...
        for ckpt_path in ckpt_list:
            ...
            fused_acc, local_acc = stn_precise_testing(
                stn_model=stn_model,
                ...
                max_samples=max_samples,
                wca_temperature=wca_temperature,
            )
            results.append((ckpt_name, fused_acc, local_acc))

        # 汇总
        print(f"\n{'=' * 60}")
        print(f"  测试汇总")
        print(f"{'=' * 60}")
        print(f"  {'Fused':>8s}  {'Local':>8s}  {'Delta':>8s}  检查点")
        print(f"  {'─' * 8}  {'─' * 8}  {'─' * 8}  {'─' * 40}")
        for name, fused_acc, local_acc in results:
            delta = (local_acc or 0) - fused_acc
            local_str = f"{local_acc:.2f}%" if local_acc is not None else "N/A"
            delta_str = f"{delta:+.2f}" if local_acc is not None else "N/A"
            print(f"  {fused_acc:7.2f}%  {local_str:>8s}  {delta_str:>8s}  {name}")
        if len(results) > 1:
            best_name, best_acc, _ = max(results, key=lambda x: x[1])
            print(f"\n  最佳 Fused:  {best_acc:.2f}% ({best_name})")
            local_best = max(results, key=lambda x: x[2] if x[2] is not None else 0)
            print(f"  最佳 Local:  {local_best[2]:.2f}% ({local_best[0]})")
